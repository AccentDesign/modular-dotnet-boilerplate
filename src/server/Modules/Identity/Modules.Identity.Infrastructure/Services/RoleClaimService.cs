// --------------------------------------------------------------------------------------------------
// <copyright file="RoleClaimService.cs" company="AccentDesign">
// </copyright>
// --------------------------------------------------------------------------------------------------
...
using Modules.Identity.Core.Features.RoleClaims.Events;
        ...
        public async Task<Result<string>> UpdatePermissionsAsync(PermissionRequest request)
        {
            try
            {
                ...
                if (role != null)
                {
                    ...
                    if (role.Name == RoleConstants.SuperAdmin)
                    {
                        if (deSelectedClaims.Any(x => x.Value == global::Shared.Core.Constants.Permissions.Roles.View) ||
                            deSelectedClaims.Any(x => x.Value == global::Shared.Core.Constants.Permissions.RoleClaims.View) ||
                            deSelectedClaims.Any(x => x.Value == global::Shared.Core.Constants.Permissions.RoleClaims.Edit))
                        {
                            return await Result<string>.FailAsync(string.Format(
                                _localizer["Not allowed to deselect {0} or {1} or {2} for this Role."],
                                global::Shared.Core.Constants.Permissions.Roles.View,
                                global::Shared.Core.Constants.Permissions.RoleClaims.View,
                                global::Shared.Core.Constants.Permissions.RoleClaims.Edit));
                        }
                    }
